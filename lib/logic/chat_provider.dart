...
class ChatProvider extends ChangeNotifier {
  ...
  Future<void> setModel({required String prompt,required bool isTextOnly}) async {

    if (isTextOnly) {

      if (prompt.trim().isEmpty) {
        print("⚠️ Error: The prompt cannot be empty.");
        return;
      }

      final model = GenerativeModel(
        model: 'gemini-1.5-flash',  // Ensure this model exists
        apiKey: getApiKey(),
  ...
  Future<void> sentMessage({
    ...
    // set the model
    await setModel(isTextOnly: isTextOnly, prompt: message);
  ...
  Future<List<Content>> getHistory({required String chatId}) async {
    List<Content> history = List.empty(growable: true);
    if (currentChatId.isNotEmpty) {
